	...
	if(m_pCmd)
		...
		if(pDoc->m_selectArray.GetSize() == 1) 
		{
			...
			if(pSelEntity->GetType() == etText) 
			{
				...
				/*以下部分为组合中文本修改的部分代码*/
			if (pSelEntity->GetType() == etComb) //组合文本修改处@！！！
			{
				CComb* pComb = static_cast<CComb*>(pSelEntity);
				for (MEntity* element : pComb->GetEntities())  //遍历组合中的每一个元素
				{
					if (element->GetType() == etText)   //文本元素则可以实现修改
					{
						MText* textObject = static_cast<MText*>(element);
						Position LeftTopPos=textObject->GetLeftTopPos(); //获取文本框的上下对角坐标
						Position GetRightBottomPos = textObject->GetRightBottomPos();
						CRect textRect(LeftTopPos.x, GetRightBottomPos.y, GetRightBottomPos.x, LeftTopPos.y);//定义一个矩形便于判断边界	
						if (textRect.PtInRect(CPoint(pos.x, pos.y))==true)
						{
							...
							dlg.m_text = oldText;
							dlg.DoModal();
							((MText*)element)->SetText(dlg.m_text);
							//}
